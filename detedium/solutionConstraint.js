function applySolutionConstraint() {
  ...
  new Array(numWords-1).fill(0).forEach((_,wordIndex)=> {
    ...
    byg.forEach((letter, letterIndex)=> {
      ...
      if (letter==="g" && previousGuess[letterIndex] !== thisGuess[letterIndex]) {
        square.className = addClassName(square.className, "redHighlight");
        thisGuess = thisGuess.slice(0, letterIndex) + "_" + thisGuess.slice(letterIndex+1);
      }
    });
    byg.forEach((letter, letterIndex)=> {
      const square = document.getElementById(`prompt-${wordIndex}-${letterIndex}`);
      if (letter==="y") {
        if (!thisGuess.includes(previousGuess[letterIndex])) {
          ...
          thisGuess = thisGuess.slice(0, indexOfMatch) + "_" + thisGuess.slice(indexOfMatch+1);
        }
      }
    });
  })
